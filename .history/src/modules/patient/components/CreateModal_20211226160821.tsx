...
function CreateModal({ modal, setModal, patientdata }:ICreateModal):ReactElement {
  ...
  const uploadData = ():void => {
    const toUpload = {
      name: '', phone: '', age: '', address: '', regNum: '', treatments: '', total: 0, medicine: '', images: '', date: startDate,
    };
  ...
  React.useEffect(() => {
    if (patientdata?.name) {
      ...
      const medIds: string[] = [];
      setChosenMed(patientdata.medicine.map((e, i) => {
        medIds.push(e.id);
        return {
          id: e.id,
          mname: e.mname,
      ...
      setImages(patientdata.images.map((e) => ({ dataURL: `${url}/patients/images/${e}.jpg` })));
    }
