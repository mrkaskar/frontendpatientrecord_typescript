...
function CreateModal({ modal, setModal, patientdata }:ICreateModal):ReactElement {
  ...
  const uploadData = (type:string):void => {
    const toUpload = {
      name: '', phone: '', age: '', address: '', regNum: '', treatments: '', total: 0, medicine: '', images: '', date: startDate, oldTotal: 0, folderId: '',
    };
    ...
    if (type === 'create') { saveP.mutate(toUpload); } else if (patientdata.total) {
